...
{
static int bfd_inited = 0 ;	//Static var to keep track of the initialization state.
	bfd *bfd_h;

	//Before you can use libbfd, a call bfd_init() to initialize libbfd’s internal state.
	if (!bfd_inited)
	{
		bfd_init();
		bfd_inited = 1 ;
	...
	bfd_h = bfd_openr(fname.c_str(),NULL);	//1nd parameter = NULL to let bfd_openr determine the binary type.
	if (!bfd_h)
	{
		fprintf(stderr, "[!!] Failed to open binary '%s' (%s)",fname.c_str(),bfd_errmsg(bfd_get_error()));
		return NULL;
	}
  //We check the handle to verifi if it's an object. 
  //which in libbfd terminology means an executable, a relocatable object, or a shared library. 
 ...
 //
 //This function returns a bfd_flavour object, which simply indicates the kind of binary (ELF, PE, and so on).
...
/*
 * Function's job is to parse a binary ﬁle speciﬁed by ﬁlename and load it into the Binary object given to it.
 * */
int load_binary (std::string &fname , Binary *bin, Binary::BinaryType type)
{
	return load_binary_bfd(fname,bin,type);	//using the bfd libarry function to do the work.
}


/*
 * Unlike load_bianry , this function does the absolute opposite.
 * */
void unload_binary(Binary *bin)
{
	size_t i ; //Alias of one of the fundamental unsigned integer types.
	Section *sec;
	//Iterating over the sections freeing them.
	for(unsigned int i = 0 ; i < bin->sections.size(); i++)
	{
		sec = &(bin->sections[i]);
		if (sec->bytes) free(sec->bytes);	//Bytes member = raw bytes in the section.
	}
}
